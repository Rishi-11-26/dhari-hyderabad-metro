...
st.markdown("### ⏰ Metro Service Status")

if metro_service_status():

    st.success("🚇 Metro service is currently operational")

else:

    st.error("🚫 Metro service is closed now")

    st.info("Operating Hours: 6:00 AM – 11:00 PM")
...
if st.button("Use My Location"):
    ...
    if location:
        ...
        st.warning("Location access denied")
...
if "route" in st.session_state:
    ...
    if route:
        ...
        for i in range(len(route)):
            ...
            current_line = row.iloc[0]["line"]

            if i==0:
                st.write(f"Start at **{station}** on **{current_line} Line**")

            elif i < len(route)-1:

                next_station = route[i+1]

                next_row = stations_df[stations_df["station"]==next_station]

                next_line = next_row.iloc[0]["line"]

                if current_line != next_line:

                    st.warning(
                        f"Interchange at **{station}** to **{next_line} Line**"
                    )

            else:
                st.success(f"Arrive at **{station}**")
